...
本项目是围绕 **PP-OCRv6** 搭建的本地 OCR 工作台。PP-OCRv6 是飞桨最新的三档 OCR 模型家族（Tiny / Small / Medium），三档模型可以在本机一键切换，同时支持 [OmniDocBench](https://github.com/opendatalab/OmniDocBench) 标准评测集的本地跑分。
...
脚本从 GitHub Releases 页面下载三档模型压缩包并解压到对应目录，
...
这套工具的测试流程，先用 FastAPI 服务把三档 ONNX 模型挂起来，然后用 `bench_local_v2.py` 把 OmniDocBench 的 18 张图逐一打过去，记录每张的编辑距离分数。真实场景测试图用 `gen_result_vis.py` 生成带检测框的可视化面板，方便直观对比不同档位的识别效果。

核心架构，FastAPI 后端负责接收图片、调用 ONNX Runtime 推理（Apple Silicon 自动走 CoreML 加速）、返回检测框坐标和识别文字。前端是一个单页应用，不依赖任何前端框架，直接 fetch API 拿数据渲染。
